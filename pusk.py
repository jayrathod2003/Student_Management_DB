...
import time
import ttkthemes
from tkinter import ttk, messagebox, Entry, filedialog
import pandas
import pymysql
#functionality part
...
def export_data():
	...
	messagebox.showinfo('Success', 'Datais saved successfuly')
...
def add_data():
	if (idEntry.get() == '' or nameEntry.get() == '' or phoneEntry.get() == '' or emailEntry.get() == '' or addressEntry.get() == '' or genderEntry.get() == '' or dobEntry.get() == ''):
		...
		try:
			query = 'insert into student values(%s,%s,%s,%s,%s,%s,%s,%s,%s)'  #query = "INSERT INTO your_table_name (id, name, phone, email, address, gender, dob, date, time) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
...
def connect_database():
	def connect():
		...
		try:
			...
			mycursor.execute(query)
			query = 'CREATE table student(id int NOT NULL primary key, name VARCHAR(30), mobile VARCHAR(10), email VARCHAR(30), address VARCHAR(120), gender VARCHAR(20), dob VARCHAR(20), date VARCHAR(50), time VARCHAR(50))'
			mycursor.execute(query)
		except:
			query = 'use studentmanagementsystem'
			mycursor.execute(query)
...
sliderLabel = Label(root, font=('arial', 28, 'italic bold'), width=38)
...
leftFrame = Frame(root, bg="#B0E0E6")
leftFrame.place(x=50, y=80, width=300, height=600)

#logo_image = PhotoImage(file="trystud.png")
...
style.configure('Treeview', rowheight=40, font=('arial', 12, 'bold'), foreground='black', fieldbackground="#FFCCCC")
